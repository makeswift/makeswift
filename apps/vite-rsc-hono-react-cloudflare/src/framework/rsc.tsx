...
import { isMutationAction } from './mutation'
...
export type RscPayload = {
  ...
  // `null` if a call don't affect the rendered tree
  root: ReactNode | null
...
export async function handleRenderRequest(
  ...
  // plain navigations always render; action calls only re-render when explicitly
  // tagged as a mutation
  const root = needsRerender ? await renderRoot(renderRequest.request) : null
...
export async function renderRscElement(
  ...
  const rscStream = renderToReadableStream(await renderElement(req))
  ...
  if (actionId) {
    ...
    try {
      ...
      return {
        ...
        needsRerender: isMutationAction(action),
      }
    } catch (e) {
