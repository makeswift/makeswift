...
function contentstackToSitemapEntry(blog: BlogPostFromQuery): NextSitemapItem {
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const makeswiftPages = await client
    ...
    .toArray()
  const contentstackPages = (await getAllBlogs()).map(blog => contentstackToSitemapEntry(blog))
