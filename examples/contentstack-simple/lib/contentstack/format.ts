...
export const formatBlog = (
  ...
  if (!blog.title || !blog.feed_date || (includeBody && !blog.body)) {
    console.log('Missing some required fields:', {
      title: blog.title,
      feed_date: blog.feed_date,
      body: includeBody ? !!blog.body : 'not required',
      banner: blog.bannerConnection?.edges?.[0]?.node?.url || 'none',
    })
