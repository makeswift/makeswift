    ...
    <div className={`${className} grid grid-cols-1 gap-6`}>
      <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-2 justify-center items-center">
          <Image src={image.url} alt={image.altText} width="80" height="80" />
          <Image src={image.url} alt={image.altText} width="80" height="80" />
          <Image src={image.url} alt={image.altText} width="80" height="80" />
        </div>
      </div>
...
export function ProductName({ className }: ProductNameProps) {
  ...
  return <div className={`${className} text-[44px] text-black font-light`}>{product.title}</div>
}
...
export function ProductDescription({ className }: ProductDescriptionProps) {
  ...
  return (
    <div className={`${className} text-lg text-black/70 font-light`}>{product.description}</div>
  )
}
