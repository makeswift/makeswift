...
import { type BlogPostFromQuery, formatBlogs } from '@/lib/dato/format'
...
export function BlogPostFeed({ className, itemsPerPage, showPagination }: Props) {
  const [currentPage, setCurrentPage] = useState(0)
  const [totalPages, setTotalPages] = useState(0)
  const skip = currentPage * itemsPerPage

  const { data, error, isLoading } = useSWR<BlogFeedResponse>(
    `/api/dato/feed?limit=${itemsPerPage}&skip=${skip}`,
    fetcher,
    {
