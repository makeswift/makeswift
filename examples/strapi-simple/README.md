...
After creating your admin account, seed the database with sample content:

```bash
npm run seed:example
```

This adds sample blog articles, authors, and categories to get you started quickly.

**Important:** Make sure to keep the Strapi server running on port 1337 in order to work with Strapi CMS content.
...
Your site will be available at `http://localhost:3000`. Make sure both the Strapi server (port 1337) and development server (port 3000) are running.
...
- **cover** (Media) - Hero image
- **blocks** (Dynamic Zone) - Article content with components:
  - **shared.rich-text** - Rich text content with a `body` field
  - **shared.media** - Media content with a `file` field

### Additional Content Types

- **Author** - Author information with name, email, and avatar
- **Category** - Article categories for organization
- **Global** - Site-wide settings and SEO defaults

The seed data creates sample content for all these types, so you can start building immediately without manual content creation.
