...
import { type BlogPostData, formatBlogs } from '@/lib/payload/format'
...
export const BlogFeed = ({ className, itemsPerPage, showPagination }: Props) => {
  ...
  const { data, error, isLoading } = useSWR<BlogFeedResponse>(
    `/api/payload/feed?limit=${itemsPerPage}&skip=${skip}`,
    fetcher,
    {
  ...
  const blogPosts = useMemo(() => formatBlogs(data?.blogs ?? []), [data])
