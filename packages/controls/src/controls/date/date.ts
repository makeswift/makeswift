...
import { AcceptedDateDataTypes, ControlDataTypeKey } from '../../common'
import { DateDataTypes } from '../../common/data-types'
...
/** Stores an ISO 8601 date string. When `includeTime` is set, the builder
 * panel also exposes a time picker; the stored value is still an ISO string.
 * Promotes the legacy `@makeswift/prop-controllers` `Date` controller — its
 * `prop-controllers::date::v1` data is accepted on read for backward
 * compatibility. */
...
class Definition<C extends Config> extends ControlDefinition<
  ...
  ResolvedValueType<C>
> {
  private static readonly v1DataType = DateDataTypes.date
  private static readonly dataSignature = {
    v1: { [ControlDataTypeKey]: this.v1DataType },
  } as const

  static readonly type = 'makeswift::controls::date' as const

  static get schema() {
    const version = z.literal(1).optional()

    const versionedData = z.object({
      [ControlDataTypeKey]: z.enum(AcceptedDateDataTypes),
      value: z.string(),
    })
  ...
  static deserialize(data: DeserializedRecord): DateDefinition {
    if (data.type !== Definition.type) {
      throw new Error(`Date: expected type ${Definition.type}, got ${data.type}`)
    }

    const { version, config } = Definition.schema.relaxed.definition.parse(data)
    return new DateDefinition(config, version)
  }
  ...
  fromData(data: DataType<C> | undefined): ValueType<C> | undefined {
    ...
    return match(data satisfies z.infer<typeof inputSchema>)
      .with(
        { [ControlDataTypeKey]: P.union(...AcceptedDateDataTypes) },
        ({ value }) => value,
      )
      .otherwise((val) => val)
  }
  ...
  accept<R>(visitor: ControlDefinitionVisitor<R>, ...args: unknown[]): R {
    return visitor.visitDate(this, ...args)
  }
}

export class DateDefinition<C extends Config = Config> extends Definition<C> {}
...
export function Date<D extends Config['defaultValue']>(
  config?: UserConfig<D>,
): DateDefinition<NormedConfig<D>> {
  return new DateDefinition((config ?? {}) as NormedConfig<D>, 1)
}
