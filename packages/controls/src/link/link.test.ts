...
describe('Link', () => {
  ...
  describe('copyData', () => {
    ...
    test('[OPEN_PAGE] links keep original `pageId` if found in replacement context', () => {
      // Act
      // Act
      const result = Link().copyData(
        {
    ...
    test('[SCROLL_TO_ELEMENT] links keep original `elementKey` if found in replacement context', () => {
      const result = Link().copyData(
        {
          type: 'SCROLL_TO_ELEMENT',
          payload: {
