...
export function TextElement({ descendant }: TextProps) {
  ...
  return (
    <>
      {styleElement}
      <span className={className}>{descendant.text === '' ? '\uFEFF' : descendant.text}</span>
    </>
  )
}
...
function InlineElement({ descendant }: InlineProps) {
  const { className: linkClassName, styleElement: linkStyleElement } = useStyle({
    textDecoration: 'none',
  })
...
export function BlockElement({ descendant }: BlockProps) {
  const responsiveBlockStyles = useResponsiveStyle(
    [descendant.textAlign],
    ([textAlign = 'left']) => ({ textAlign }),
  )
  const blockStyles: CSSObject = { margin: 0, ...responsiveBlockStyles }
  const { className: blockStylesClassName, styleElement: blockStylesStyleElement } =
    useStyle(blockStyles)
  const quoteStyles = {
    padding: '0.5em 10px',
    fontSize: '1.25em',
    fontWeight: '300',
