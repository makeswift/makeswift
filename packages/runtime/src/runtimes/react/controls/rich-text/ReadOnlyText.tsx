...
function InlineElement({ descendant }: InlineProps) {
  const { className: linkClassName, renderStaticStyle } = useStyle({ textDecoration: 'none' })
...
export function BlockElement({ descendant }: BlockProps) {
  ...
  const { className: blockStylesClassName, renderStaticStyle: renderBlockStyles} = useStyle(blockStyles)
  const quoteStyles = {
    padding: '0.5em 10px',
    fontSize: '1.25em',
    fontWeight: '300',
