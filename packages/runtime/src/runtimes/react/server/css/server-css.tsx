...
import { StylesheetEngine, MAKESWIFT_RSC_STYLE_HREF_ID } from './css-runtime'
...
export function InjectServerCSS({ collector }: { collector: ServerCSSCollector }) {
  ...
  return (
    // `precedence` and `href` props are required to opt the `<style>` element into React's stylesheet
    ...
    <style href={MAKESWIFT_RSC_STYLE_HREF_ID} precedence="makeswift">
      {css}
    </style>
  )
}
