...
describe('getGlobalElements', () => {
  ...
  test('sends all ids as repeated query params, omits locale by default, and returns the response in order', async () => {
    ...
    const handler = jest.fn(({ request }: { request: Request }) => {
      ...
      return HttpResponse.json(
        ids.map(id => ({
          globalElement: id === 'b' ? null : { id, data: {} },
    ...
    expect(result).toEqual([
      { globalElement: { id: 'a', data: {} }, localizedGlobalElement: null },
      { globalElement: null, localizedGlobalElement: null },
      { globalElement: { id: 'c', data: {} }, localizedGlobalElement: null },
    ])
  })
  ...
  test('sends the locale as a query param when provided', async () => {
    ...
    const handler = jest.fn(({ request }: { request: Request }) => {
      ...
      return HttpResponse.json(
        ids.map(id => ({
          globalElement: { id, data: {} },
          localizedGlobalElement: { id: `${id}-es-MX`, data: {} },
        })),
      )
    })
    server.use(http.get(resourceUrl, handler, { once: true }))
    ...
    expect(result).toEqual([
      { globalElement: { id: 'a', data: {} }, localizedGlobalElement: { id: 'a-es-MX', data: {} } },
    ])
  })
