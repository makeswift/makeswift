...
export type ApiHandlerInternalConfig = {
  ...
  notFoundHandler?: (path: string) => Promise<Response | void>
}
...
type ApiHandler = (req: ApiRequest, res: ApiResponse, route: string) => Promise<ResponseType>
...
export function createApiHandler(
  ...
  return async function (req: ApiRequest, res: ApiResponse, route: string): Promise<ResponseType> {
    ...
    if (req.method.toUpperCase() === 'OPTIONS') {
      return res.setHeader('Content-Length', '0').status(204)
    }

    return await apiRouteHandler(req, res, route)
  }
  ...
  async function apiRouteHandler(
    ...
    const siteVersion = parseSiteVersion(req.getHeader(API_HANDLER_SITE_VERSION_HEADER))
    const action = '/' + route
    const matches = <T extends object>(pattern: string): Match<T> =>
      matchPattern<T>(pattern, { decode: decodeURIComponent })(action)

    if (matches('/clear-draft')) return clearDraftHandler(req, res, { draftCookieNames })
    if (matches('/element-tree')) return elementTreeHandler(req, res, { runtime })
    if (matches('/fonts')) return fontsHandler(req, res, { getFonts })
    if (matches('/manifest')) return manifestHandler(req, res, { apiKey, manifest })
    if (matches('/merge-translated-data')) return mergeTranslatedDataHandler(req, res, { client })
    if (matches('/revalidate')) {
      return revalidateHandler(req, res, { apiKey, revalidatePath: revalidationHandler })
    }

    if (matches('/translatable-data')) return translatableDataHandler(req, res, { client })
    if (matches('/webhook')) {
      return webhookHandler(req, res, { apiKey, events, revalidate: revalidationHandler })
    }

    const handleResource = <T extends APIResource>(
      resource: T | null,
    ): ApiResponse<APIResource | ErrorResponseBody> => {
      return resource !== null ? res.json(resource) : res.status(404).json({ message: 'Not Found' })
    }
    ...
    if ((m = matches<{ id: string }>('/page-pathname-slices/:id'))) {
      const localeParam = req.getSearchParam('locale')
    ...
    if (notFoundHandler) {
      return await notFoundHandler(action)
    }
