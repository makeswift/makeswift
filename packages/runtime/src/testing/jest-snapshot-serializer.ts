...
function getCssRulesForClassNames(classNames: Set<string>): CSSRule[] {
...
export function createMakeswiftStylesSnapshotSerializer(): jest.SnapshotSerializerPlugin {
  ...
  function serialize(val: any, config: any, indentation: any, depth: any, refs: any, printer: any) {
    visited.add(val)
    try {
      if (depth === 0) {
        ...
        const rulesToAppendToSnapshot =
          formattedRules.length > 0 ? formattedRules.join('\n') + '\n\n' : ''
