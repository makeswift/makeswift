...
function getComponentsMeta(
  state: State
): Map<string, ComponentsMeta.ComponentMeta> {
  ...
  return (dispatch, getState) => {
    ...
    measuredBoxModels.forEach((measuredBoxModel, elementKey) => {
      ...
      if (
        ...
        !deepEqual(currentBoxModel, measuredBoxModel)
      ) {
...
export function startMeasuringElements(): ThunkAction<
  () => void,
  State,
  unknown,
  Action
...
function startMeasuringDocumentElement(): ThunkAction<
  () => void,
  unknown,
  unknown,
  Action
...
export function initialize(): ThunkAction<() => void, State, unknown, Action> {
  return (dispatch) => {
    ...
    const stopMeasuringDocumentElement = dispatch(
      startMeasuringDocumentElement()
    );
...
> {
  return ({ dispatch, getState }: MiddlewareAPI<Dispatch, State>) =>
    (next: ReduxDispatch<Action>) => {
      ...
      messageChannel.port1.onmessage = (event: MessageEvent<Action>) =>
        dispatch(event.data);
