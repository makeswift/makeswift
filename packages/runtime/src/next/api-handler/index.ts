...
export function MakeswiftApiHandler(
  ...
  async function makeswiftApiHandler(
    ...
    await match(args)
      .with(routeHandlerPattern, () => Promise.resolve())
      .with(apiRoutePattern, async ([req, res]) => {
    ...
    const siteVersion = MakeswiftSiteVersion.Working
  ...
  return (...args: MakeswiftApiHandlerArgs) =>
    match(args)
      .with(routeHandlerPattern, async args => {
        const response = await makeswiftApiHandler(...args)
        const headers = new Headers(response.headers)

        headers.append('Access-Control-Allow-Origin', appOrigin)
        headers.append('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS')
        headers.append('Access-Control-Allow-Headers', 'Content-Type, Authorization')

        return new Response(response.body, { headers, status: response.status })
      })
      .with(apiRoutePattern, args => makeswiftApiHandler(...args))
      .exhaustive()
}
