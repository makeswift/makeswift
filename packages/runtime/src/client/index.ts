  ...
  private async introspectMany(
    ...
    const traversals = new Map(
      trees.map(tree => [
        tree.id,
        {
          ...
          remaining: [tree.data] as Element[],
          seen: new Set<string>(),
        },
      ]),
    )
    ...
    while (true) {
      ...
      ids.forEach((id, index) => {
        globalElementCache.set(id, results[index]?.globalElement ?? null)
        localizedGlobalElementCache.set(id, results[index]?.localizedGlobalElement ?? null)
      })
