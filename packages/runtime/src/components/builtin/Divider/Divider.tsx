...
const Divider = forwardRef(function Divider(
  ...
  const outerDivStyles = composeStyles(useStyle({ display: 'flex', width: '100%' }), width, margin)
    ...
    useResponsiveStyle(
      ...
      ([
        ...
        switch (variant) {
          ...
          case 'dotted':
            return {
              borderBottomWidth: thickness == null ? 1 : `${thickness.value}${thickness.unit}`,
  ...
  return (
    ...
    <>
      ...
      <div ref={ref} id={id} className={outerDivStyles.className}>
        {flexColumnDivStyle.styleElement}
        <div className={flexColumnDivStyle.className}>
          {responsiveBorderStyle.styleElement}
          <div className={responsiveBorderStyle.className} />
        </div>
      </div>
    </>
  )
