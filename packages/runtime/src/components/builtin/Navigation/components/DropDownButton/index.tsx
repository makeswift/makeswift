...
function DropDownMenu({ className, position, ...restOfProps }: DropDownMenuProps) {
  const { className: dropDownMenuStaticStylesClassName, styleElement: dropdownMenuStylesElement } =
    useStyle({
      position: 'absolute',
      top: '100%',
  ...
  return (
    <>
      ...
      <div
        {...restOfProps}
        className={clsx(DROP_DOWN_MENU_CLASS_NAME, dropDownMenuStaticStylesClassName, className)}
      />
    </>
  )
}
...
const DropDownContainer = forwardRef(function DropDownContainer(
  ...
  const { keyframesName: dropInKeyframesName, styleElement: dropInKeyframeStyleElement } =
    useKeyframes(`
    0% {
        opacity: 0;
  ...
  return (
    <>
      ...
      {baseStyleElement}
      <div {...restOfProps} ref={ref} className={clsx(baseClassName, className)} />
    </>
  )
})
...
function DropDownItem({ className, color, textStyle, ...restOfProps }: DropDownItemProps) {
  ...
  return (
    <>
      ...
      <Link {...restOfProps} className={styles.className} />
    </>
  )
}
