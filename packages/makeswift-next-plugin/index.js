  ...
  async function isFileAMakeswiftComponent(file) {
    ...
    for (const moduleItem of module.body) {
      if (moduleItem.type === "ExportDeclaration") {
        if (moduleItem.declaration.type === "VariableDeclaration" && moduleItem.declaration.kind === "const") {
            ...
            if (firstItem.id.type === "Identifier") {
              if (firstItem.id.value === "makeswift") {
                if (firstItem.init != null && firstItem.init.type === "ObjectExpression") {
                  ...
                  const filteredProperties = properties.filter(property => property.type === "KeyValueProperty" && property.key.type === "Identifier" && property.key.value === "type")

                  if (filteredProperties.length === 0) {
                    throw Error(`The 'type' property is not defined in your 'makeswift' const export in file ${file}`)
                  } else if (filteredProperties.length > 1) {
                    throw Error(`The 'type' property is defined multiple times in your 'makeswift' const export in file ${file}`)
                  }
